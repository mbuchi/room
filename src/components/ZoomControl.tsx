...
const ZoomControl = ({ getMap, isDarkMode, className = '', rightOffsetPx = null, align = 'right' }: ZoomControlProps) => {
  ...
  // At glass level >0 use the shared translucent `.glass-control`; at Off keep
  // the original solid card so the default look is byte-identical.
  const panel = glassOn
    ? `glass-control ${isDarkMode ? 'text-slate-200' : 'text-slate-700'}`
    : isDarkMode
