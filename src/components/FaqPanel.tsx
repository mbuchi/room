...
const FaqPanel = ({
  ...
  return (
    <PanelScroll actionsSlot={actionsSlot}>
      <div className="rounded-lg px-4 py-3.5 bg-slate-50 ring-1 ring-slate-200/80 dark:bg-white/[0.035] dark:ring-white/6">
        ...
        <button
          type="button"
          onClick={onAskClaire}
          className="mt-2.5 w-full min-w-0 flex items-center justify-center gap-2 min-h-11 rounded-xl px-4 py-2.5 text-sm font-semibold bg-slate-100 text-slate-800 ring-1 ring-slate-200 hover:bg-slate-200/70 dark:bg-white/5 dark:text-slate-200 dark:ring-white/[0.07] dark:hover:bg-white/8 transition active:scale-[0.99]"
        >
      ...
      <div className="space-y-2">
        {qas.map(({ q, a }) => (
          <details
            key={q}
            className="group rounded-lg overflow-hidden bg-slate-50 ring-1 ring-slate-200/80 dark:bg-white/[0.035] dark:ring-white/6"
          >
            <summary className="flex items-center justify-between gap-2 cursor-pointer list-none px-4 py-2.5 text-[11px] font-medium text-gray-700 dark:text-gray-200 select-none hover:bg-slate-100 dark:hover:bg-white/6 transition-colors">
              <span>{q}</span>
              <ChevronDown
                size={13}
                className="shrink-0 text-gray-400 dark:text-gray-500 transition-transform group-open:rotate-180"
              />
            </summary>
